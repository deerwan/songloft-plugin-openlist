...
- **页面试听**:点击音频文件直接播放(内置迷你播放器,队列=当前目录全部音频,支持上下首/进度/连续坏源跳过),无需先入库
- **入库与歌单**:多选音频文件后直接入库,或导入到新建 / 已有歌单
- **播放解析**:播放时实时解析直链——`raw_url` 指向 OpenList 自身(代理/本地)时直接使用;指向第三方网盘 CDN 的快照直链因可能过期,改走 `/d/<path>?sign=` 每次重新解析新直链
- **全局搜索**:聚合所有已配置服务器的 `fs/search`,接入 Songloft 搜索
- **封面与歌词**:代理 OpenList 的 thumb 封面,经 `fs/get` 的 `related` 字段查找同级同名 `.lrc` 歌词
